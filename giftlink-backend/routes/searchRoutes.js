...
query.category = req.query.category;        if (req.query.condition) {
            // {{insert code here}} 
        }
query.condition = req.query.condition;        if (req.query.age_years) {
            // {{insert code here}}
        }
...
const gifts = await collection.find(query).toArray();        res.json(gifts);
    } catch (e) {
        next(e);
    }
});
